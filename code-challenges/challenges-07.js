...
const objLat = (obj) => {
    ...
    let fLetter = fName.slice(0, 1);
    let lastLetters = fName.slice(1)
    fName = fLetter.toUpperCase() + lastLetters;
    let lName = obj.lastName;
    fLetter = lName.slice(0, 1);
    lastLetters = lName.slice(1)
    lName = fLetter.toUpperCase() + lastLetters;
...
const cvFormatter = (arr) => {
    ...
    //     }
    //     let tech=arr[i].tech;
    //     let newCv= new object(fullName,tech);
    //     newArr.push(newCv);
    //     }
    // }
    // return newArr;
    ...
    for (let i = 0; i < arr.length; i++) {
        if (arr[i].yearsOfExperience > 1) {
            let fName = arr[i].firstName + " " + arr[i].lastName;
            ...
            newArr.push({ fullName: fName, tech: arr[i].tech })
        }
    }
    return newArr;
}
