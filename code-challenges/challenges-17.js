...
const recursionPattern = (int1, int2) => {
    ...
    function myFunction(a, b) {
        if (a === -4) {
            function continueFunction(a, b) {
                if (a === int1) {
                    return arr.push(a);
                } else {
                    arr.push(a);
                    return continueFunction(a + b, b);
                }
            }
            continueFunction(a, b);

        } else {
            arr.push(a);
            return myFunction(a - b, b);
        }
    }
    myFunction(a, b);
    return arr;
...
const filterLinks = (str) => {
    // write your code here
    const regex1 = /www/g;
    const regex2 = /\.(com|org|net)\b/g;
    let locationOfHttp = str.search(regex1);
    let locationOfDot = str.search(regex2);
    return str.slice(locationOfHttp, locationOfDot + 4);
}
// -------------------------------------------------------------------------------------------------------
...
// Given a string s, return true if it is a palindrome, or false otherwise.dwwdwdwwdw
// 
...
const isPalindrome = (str) => {
    ...
    let lettersArr = smallLetters.split("");
  let lettersOnly = lettersArr.filter((e) => /^[a-zA-Z]+$/.test(e));
    if (lettersOnly.join() === lettersOnly.reverse().join()) {
        return true;
    } else {
        return false;
    }
